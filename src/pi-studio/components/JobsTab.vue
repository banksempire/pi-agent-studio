...
function rowClass(row: Record<string, unknown>): Record<string, boolean> {
  return {
    ...
    'jobs-row--sel': row.id === store.selectedJob?.id,
  };
}
...
function openEdit(row: Record<string, unknown>) {
  store.openJobEditor(row.id as string);
}

function closeDialog() {
  store.closeJobEditor();
}

const editorJob = computed(() =>
  store.jobEditor.open && store.jobEditor.jobId
    ? (store.jobs.find((j) => j.id === store.jobEditor.jobId) ?? null)
    : null,
);
